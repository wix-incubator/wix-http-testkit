...
object Request extends HttpMessageType("Request") {
  override def isCookieHeader(header: HttpHeader): Boolean = header.isInstanceOf[Cookie]
}

trait RequestHeadersMatchers extends HeaderMatching[HttpRequest] {
  override protected val httpMessageType = Request
}
